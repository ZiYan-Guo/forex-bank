...
try {
  ...
  graph.nodes.forEach((n, i) => {
    ...
    if (seen.has(n.id)) issues.push(`Duplicate node ID '${n.id}' at indices ${seen.get(n.id)} and ${i}`); else seen.set(n.id, i);
    nodeIds.add(n.id);
  });
  ...
  graph.layers.forEach(layer => {
    for (const field of ['id','name','description','nodeIds']) if (layer[field] == null) issues.push(`Layer missing field ${field}`);
  ...
  graph.tour.forEach((step, i) => {
    for (const field of ['order','title','description','nodeIds']) if (step[field] == null) issues.push(`Tour step[${i}] missing field ${field}`);
